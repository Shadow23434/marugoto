(function () {
  ...
  function init() {
    const PAGE_SCOPE = document.querySelector("main#katakana");
    if (!PAGE_SCOPE) return;

    const JSON_URL = "data/en/katakana.json";
    const SELECTOR_TRIGGER = "#katakana .char_list a.inline_modal";
    ...
    // Resolve remote vocab image URL to local path under assets/images/katakana/vocabularies
    function localVocabImgPath(remoteUrl) {
      ...
      if (!base) return "";
      return `assets/images/katakana/vocabularies/${base}`;
    }

    // Resolve remote stroke order image URL to local path under assets/images/katakana/order
    function localOrderImgPath(remoteUrl) {
      ...
      if (!base) return "";
      return `assets/images/katakana/order/${base}`;
    }
    ...
    function getData() {
      if (!dataPromise) {
        dataPromise = fetch(JSON_URL).then((r) => {
          if (!r.ok) throw new Error("Failed to load katakana.json");
          return r.json();
        });
      }
      return dataPromise;
    ...
    function renderCharacter(container, rowChars, activeIndex) {
      ...
      const mp4 = romaji
        ? `assets/movies/katakana/animation/mp4/${romaji}.mp4`
        : "";
      const webm = romaji
        ? `assets/movies/katakana/animation/webm/${romaji}.webm`
        : "";
      const audioFile = ch.audio
        ...
        ? `${romaji}.mp3`
        : "";
      const localCharAudio = audioFile
        ? `assets/sounds/common/${audioFile}`
        : "";
