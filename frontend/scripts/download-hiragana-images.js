#!/usr/bin/env node
/**
 * Download all character images referenced in hiragana.json (characterImgUrl fields).
 *
 * Source fields:
 *   - (data/<lang>/hiragana.json).groups[*].characters[*].characterImgUrl
 *
 * Default output:
 *   assets/images/hiragana/chracters/<filename>
 *   (note: folder name kept as requested: "chracters")
 *
 * Usage (PowerShell on Windows):
 *   node scripts/download-hiragana-images.js
 *   node scripts/download-hiragana-images.js --lang en
 *   node scripts/download-hiragana-images.js --lang all
 *   node scripts/download-hiragana-images.js --input data/en/hiragana.json
 *   node scripts/download-hiragana-images.js --outDir assets/images/hiragana/chracters
 *   node scripts/download-hiragana-images.js --concurrency 6 --retries 2 --force
 *   node scripts/download-hiragana-images.js --limit 10
 *
 * Flags:
 *   --input <path>     Path to a specific hiragana.json (overrides --lang)
 *   --lang <code>      One of en|ja|es|all (default: en)
 *   --outDir <path>    Destination directory (default: assets/images/hiragana/chracters)
...
function parseArgs(argv) {
  const args = {
    lang: "en",
    outDir: path.join("assets", "images", "hiragana", "form"),
    force: false,
    concurrency: 6,
    retries: 2,
    delay: 200,
...
function extractCharacterImgUrls(hiraganaJson) {
  const urls = new Set();
...
async function main() {
  ...
  for (const input of args.inputs) {
    ...
    try {
      ...
      const urls = extractCharacterImgUrls(hira);
