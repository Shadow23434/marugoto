#!/usr/bin/env node
/**
 * Download all character images referenced in katakana.json (characterImgUrl fields).
 *
 * Source fields:
 *   - (data/<lang>/katakana.json).groups[*].characters[*].characterImgUrl
 *
 * Default output:
 *   assets/images/katakana/form/<filename>
 *
 * Usage (PowerShell on Windows):
 *   node scripts/download-katakana-images.js
 *   node scripts/download-katakana-images.js --lang en
 *   node scripts/download-katakana-images.js --lang all
 *   node scripts/download-katakana-images.js --input data/en/katakana.json
 *   node scripts/download-katakana-images.js --outDir assets/images/katakana/form
 *   node scripts/download-katakana-images.js --concurrency 6 --retries 2 --force
 *   node scripts/download-katakana-images.js --limit 10
 *
 * Flags:
 *   --input <path>     Path to a specific katakana.json (overrides --lang)
 *   --lang <code>      One of en|ja|es|all (default: en)
 *   --outDir <path>    Destination directory (default: assets/images/katakana/form)
...
function parseArgs(argv) {
  const args = {
    lang: "en",
    outDir: path.join("assets", "images", "katakana", "form"),
    force: false,
    concurrency: 6,
    retries: 2,
    delay: 200,
  ...
  if (!args.input) {
    if (args.lang === "all") {
      args.inputs = [
        path.join("data", "en", "katakana.json"),
        path.join("data", "ja", "katakana.json"),
        path.join("data", "es", "katakana.json"),
      ];
    } else {
      args.inputs = [path.join("data", args.lang, "katakana.json")];
    }
  } else {
...
async function readKatakanaJson(jsonPath) {
...
function extractCharacterImgUrls(katakanaJson) {
  const urls = new Set();
  const groups = Array.isArray(katakanaJson.groups) ? katakanaJson.groups : [];
  for (const g of groups) {
    const chars = Array.isArray(g.characters) ? g.characters : [];
    for (const item of chars) {
      const url =
        item && typeof item.characterImgUrl === "string"
          ? item.characterImgUrl.trim()
          : "";
      if (url) urls.add(url);
    }
  }
...
async function main() {
  ...
  for (const input of args.inputs) {
    ...
    try {
      const data = await readKatakanaJson(inputPath);
      const urls = extractCharacterImgUrls(data);
  ...
  console.log(`[info] Found ${urls.length} character image URLs.`);
