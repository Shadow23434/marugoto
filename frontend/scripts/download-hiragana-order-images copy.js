#!/usr/bin/env node
/**
 * Download all stroke order images referenced in hiragana.json (orderImgUrl fields).
 *
 * Source fields:
 *   - (data/<lang>/hiragana.json).groups[*].characters[*].orderImgUrl
 *
 * Default output:
 *   assets/images/hiragana/order/<filename>
 *
 * Usage (PowerShell on Windows):
 *   node scripts/download-hiragana-order-images.js
 *   node scripts/download-hiragana-order-images.js --lang en
 *   node scripts/download-hiragana-order-images.js --lang all
 *   node scripts/download-hiragana-order-images.js --input data/en/hiragana.json
 *   node scripts/download-hiragana-order-images.js --outDir assets/images/hiragana/order
 *   node scripts/download-hiragana-order-images.js --concurrency 6 --retries 2 --force
 *   node scripts/download-hiragana-order-images.js --limit 10
 *
 * Flags:
 *   --input <path>     Path to a specific hiragana.json (overrides --lang)
 *   --lang <code>      One of en|ja|es|all (default: en)
 *   --outDir <path>    Destination directory (default: assets/images/hiragana/order)
...
function parseArgs(argv) {
  const args = {
    lang: "en",
    outDir: path.join("assets", "images", "hiragana", "order"),
    force: false,
    concurrency: 6,
    retries: 2,
    delay: 200,
...
function extractOrderImgUrls(hiraganaJson) {
  const urls = new Set();
