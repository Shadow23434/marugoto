#!/usr/bin/env node
/**
 * Download all topic images referenced in topics.json (topicImageUrl fields).
 *
 * Source fields:
 *   - (data/<lang>/topics.json).topics[*].topicImageUrl
 *
 * Default output:
 *   assets/images/topic/bg/<filename>
 *
 * Usage (PowerShell on Windows):
 *   node scripts/download-topic-images.js
 *   node scripts/download-topic-images.js --lang en
 *   node scripts/download-topic-images.js --lang all
 *   node scripts/download-topic-images.js --input data/en/topics.json
 *   node scripts/download-topic-images.js --outDir assets/images/index
 *   node scripts/download-topic-images.js --concurrency 6 --retries 2 --force
 *   node scripts/download-topic-images.js --limit 3
 *
 * Flags:
 *   --input <path>     Path to a specific topics.json (overrides --lang)
 *   --lang <code>      One of en|ja|es|all (default: en)
 *   --outDir <path>    Destination directory (default: assets/images/index)
...
function parseArgs(argv) {
  const args = {
    lang: "en",
    outDir: path.join("assets", "images", "topic", "bg"),
    force: false,
    concurrency: 6,
    retries: 2,
    delay: 200,
  ...
  if (!args.input) {
    if (args.lang === "all") {
      args.inputs = [
        path.join("data", "en", "topics.json"),
        path.join("data", "ja", "topics.json"),
        path.join("data", "es", "topics.json"),
      ];
    } else {
      args.inputs = [path.join("data", args.lang, "topics.json")];
    }
  } else {
...
async function readTopicsJson(jsonPath) {
...
function extractTopicImageUrls(topicsJson) {
  const urls = new Set();
  const arr = Array.isArray(topicsJson.topics) ? topicsJson.topics : [];
  for (const t of arr) {
    const url = t?.topicImageUrl;
...
async function main() {
  ...
  for (const input of args.inputs) {
    ...
    try {
      const topics = await readTopicsJson(inputPath);
      const urls = extractTopicImageUrls(topics);
  ...
  console.log(`[info] Found ${urls.length} topic image URLs.`);
