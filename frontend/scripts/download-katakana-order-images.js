#!/usr/bin/env node
/**
 * Download all stroke order images referenced in katakana.json (orderImgUrl fields).
 *
 * Source fields:
 *   - (data/<lang>/katakana.json).groups[*].characters[*].orderImgUrl
 *
 * Default output:
 *   assets/images/katakana/order/<filename>
 *
 * Usage (PowerShell on Windows):
 *   node scripts/download-katakana-order-images.js
 *   node scripts/download-katakana-order-images.js --lang en
 *   node scripts/download-katakana-order-images.js --lang all
 *   node scripts/download-katakana-order-images.js --input data/en/katakana.json
 *   node scripts/download-katakana-order-images.js --outDir assets/images/katakana/order
 *   node scripts/download-katakana-order-images.js --concurrency 6 --retries 2 --force
 *   node scripts/download-katakana-order-images.js --limit 10
 *
 * Flags:
 *   --input <path>     Path to a specific katakana.json (overrides --lang)
 *   --lang <code>      One of en|ja|es|all (default: en)
 *   --outDir <path>    Destination directory (default: assets/images/katakana/order)
...
function parseArgs(argv) {
  const args = {
    lang: "en",
    outDir: path.join("assets", "images", "katakana", "order"),
    force: false,
    concurrency: 6,
    retries: 2,
    delay: 200,
...
function extractOrderImgUrls(katakanaJson) {
  const urls = new Set();
  ...
  for (const g of groups) {
    ...
    for (const ch of chars) {
      const url =
        ch && typeof ch.orderImgUrl === "string" ? ch.orderImgUrl.trim() : "";
      if (url) urls.add(url);
    }
  }
...
async function main() {
  ...
  for (const input of args.inputs) {
    ...
    try {
      ...
      const urls = extractOrderImgUrls(hira);
  ...
  console.log(`[info] Found ${urls.length} order image URLs.`);
