#!/usr/bin/env node
/**
 * Download all Step1 downloadLink PDFs referenced in candos.json.
 *
 * Source fields:
 *   - steps[*] where id === 'step1' and step.question.downloadLink is a string URL
 *
 * Default output:
 *   assets/pdf/topics/<filename>
 *
 * Usage (PowerShell on Windows):
 *   node scripts/download-pdfs.js
 *   node scripts/download-pdfs.js --lang en
 *   node scripts/download-pdfs.js --lang all
 *   node scripts/download-pdfs.js --input data/en/candos.json
 *   node scripts/download-pdfs.js --outDir assets/pdf/topics
 *   node scripts/download-pdfs.js --concurrency 6 --retries 2 --force
 *   node scripts/download-pdfs.js --limit 5
 *
 * Flags:
 *   --input <path>     Path to a specific candos.json (overrides --lang)
 *   --lang <code>      One of en|ja|es|all (default: en)
 *   --outDir <path>    Destination directory (default: assets/pdf/topics)
...
function parseArgs(argv) {
  const args = {
    lang: "en",
    outDir: path.join("assets", "pdf", "topics"),
    force: false,
    concurrency: 6,
    retries: 2,
    delay: 200,
...
async function main() {
  ...
  for (const input of args.inputs) {
    ...
    try {
      ...
      const urls = extractStep1PdfLinks(candos);
  ...
  console.log(`[info] Found ${urls.length} PDF URLs.`);
