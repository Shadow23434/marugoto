#!/usr/bin/env node
/**
 * Download all katakana stroke-order animations referenced in katakana.json
 * (animation.{mp4Url, webmUrl}).
 *
 * Source fields:
 *   - (data/<lang>/katakana.json).groups[*].characters[*].animation.{mp4Url, webmUrl}
 *
 * Default output:
 *   assets/movies/katakana/animation/mp4/<filename>.mp4
 *   assets/movies/katakana/animation/webm/<filename>.webm
 *
 * Usage (PowerShell on Windows):
 *   node scripts/download-katakana-animations.js                   # en by default
 *   node scripts/download-katakana-animations.js --lang en         # explicit language
 *   node scripts/download-katakana-animations.js --lang all        # merge en+ja+es
 *   node scripts/download-katakana-animations.js --input data/en/katakana.json
 *   node scripts/download-katakana-animations.js --outDir assets/movies/katakana/animation
 *   node scripts/download-katakana-animations.js --format mp4      # only mp4
 *   node scripts/download-katakana-animations.js --limit 10        # first 10 only
 *   node scripts/download-katakana-animations.js --force           # overwrite
 */
...
function parseArgs(argv) {
  const args = {
    ...
    input: undefined,
    outDir: path.join("assets", "movies", "katakana", "animation"),
    force: false,
    concurrency: 4,
    retries: 2,
    delay: 200,
...
function extractAnimationUrls(kataJson) {
  const mp4 = new Set();
  const webm = new Set();
  const groups = Array.isArray(kataJson.groups) ? kataJson.groups : [];
...
async function main() {
  ...
  for (const input of args.inputs) {
    ...
    try {
      const kata = await readKatakanaJson(inputPath);
      const { mp4, webm } = extractAnimationUrls(kata);
