#!/usr/bin/env node
/**
 * Download all audio files referenced in hiragana.json (audio fields).
 *
 * Source fields:
 *   - (data/<lang>/hiragana.json).groups[*].characters[*].audio
 *
 * Default output:
 *   assets/sounds/common/<filename>
 *
 * Usage (PowerShell on Windows):
 *   node scripts/download-hiragana-audio.js
 *   node scripts/download-hiragana-audio.js --lang en
 *   node scripts/download-hiragana-audio.js --lang all
 *   node scripts/download-hiragana-audio.js --input data/en/hiragana.json
 *   node scripts/download-hiragana-audio.js --outDir assets/sounds/common
 *   node scripts/download-hiragana-audio.js --concurrency 6 --retries 2 --force
 *   node scripts/download-hiragana-audio.js --limit 10
 *
 * Flags:
 *   --input <path>     Path to a specific hiragana.json (overrides --lang)
 *   --lang <code>      One of en|ja|es|all (default: en)
 *   --outDir <path>    Destination directory (default: assets/sounds/common)
...
function parseArgs(argv) {
  const args = {
    lang: "en",
    outDir: path.join("assets", "sounds", "common"),
    force: false,
    concurrency: 6,
    retries: 2,
    delay: 200,
...
function extractAudioUrls(hiraganaJson) {
  const urls = new Set();
...
async function main() {
  ...
  for (const input of args.inputs) {
    ...
    try {
      ...
      const urls = extractAudioUrls(hira);
  ...
  console.log(`[info] Found ${urls.length} audio URLs.`);
